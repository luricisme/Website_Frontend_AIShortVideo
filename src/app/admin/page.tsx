...
import {
    ...
    Bell,
    ChevronRight,
    Plus, Eye, Trash2, CheckCircle, Edit, Trash,
} from 'lucide-react';
...
const DonutChart = ({ data }) => {
    ...
    const total = data.reduce((sum: never, item: { value: never; }) => sum + item.value, 0);
    let currentOffset = 0;
            ...
            <div className="py-4 grid grid-cols-2 gap-8 my-4">
                    ...
                    <Card className=" py-6 rounded-md shadow-sm ">
                        <CardContent className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex space-x-2">
                                <button
                                    className={`px-2 py-1 text-xs rounded ${activeTimeRange === 'Daily' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100'}`}
                                    onClick={() => setActiveTimeRange('Daily')}
                                >
                                    Daily
                                </button>
                                <button
                                    className={`px-2 py-1 text-xs rounded ${activeTimeRange === 'Weekly' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100'}`}
                                    onClick={() => setActiveTimeRange('Weekly')}
                                >
                                    Weekly
                                </button>
                                <button
                                    className={`px-2 py-1 text-xs rounded ${activeTimeRange === 'Monthly' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100'}`}
                    ...
                    <Card className=" py-4 rounded-md shadow-sm ">
                        <CardContent className="flex justify-between items-center mb-4 px-8">
                            ...
                            <div className="flex space-x-2">
                                <button className="px-2 py-1 text-xs rounded bg-purple-100 text-purple-600">7 Days</button>
                                <button className="px-2 py-1 text-xs rounded bg-gray-100">30 Days</button>
                            </div>
                        </CardContent>
                        ...
                        <div className="h-64 px-2">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={contentCreationData}>
                                    ...
                                    <YAxis axisLine={false} tickLine={false} />
                                    <Tooltip />
                                    <Bar dataKey="videos" name="Video Uploads" fill="#a78bfa" radius={[4, 4, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </Card>
                        ...
                        <div className="space-y-4">
                            {popularTags.map((tag, index) => (
                                <div key={index} className="flex justify-between items-center">
                                    ...
                                    <div className={`text-sm ${tag.growth.startsWith('+') ? 'text-green-500' : 'text-red-500'}`}>
                                        {tag.growth}
                                    </div>
                                </div>
                            ))}
                                        ...
                                        <TableRow key={video.id}>
                                            ...
                                            <TableCell>
                                                <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                    ...
                                                    <div className="h-2 w-2 mr-1 rounded-full ${
                            ...
                            {recentActivity.map((activity, index) => (
                                <div key={index} className="flex items-start">
                                    <div className={`p-2 rounded-full mr-3 ${
                                        activity.type === 'user-registered' ? 'bg-blue-100' :
                                            activity.type === 'video-created' ? 'bg-purple-100' :
                                                activity.type === 'content-reported' ? 'bg-yellow-100' : 'bg-green-100'
                                    }`}>
                                        {activity.type === 'user-registered' && <Users size={16} className="text-blue-500" />}
                                        {activity.type === 'video-created' && <Video size={16} className="text-purple-500" />}
                                        {activity.type === 'content-reported' && <Flag size={16} className="text-yellow-500" />}
                                        {activity.type === 'trending-tag' && <TrendingUp size={16} className="text-green-500" />}
                                    </div>

                                    <div className="flex-1">
                                        <div className="flex justify-between">
                                            <div className="font-medium">
                                                {activity.type === 'user-registered' && 'New user registered'}
                                                {activity.type === 'video-created' && 'New video created'}
                                                {activity.type === 'content-reported' && 'Content reported'}
